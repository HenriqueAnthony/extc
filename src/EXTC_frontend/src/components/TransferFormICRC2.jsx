...
import { EXTC_icrc1_ledger_canister } from 'declarations/EXTC_icrc1_ledger_canister';
...
function TransferFormICRC2() {
  ...
  async function getCanisterPrincipal(){
    const result = await EXTC_backend.getCanisterPrincipal();
  ...
  */
  async function approveTokens(am) {    
    ...
    try {
      ...
      */
      const result = await EXTC_icrc1_ledger_canister.icrc2_approve(approveArgs);    
      ...
      await authClient.login({
          ...
          */
          ...
          onSuccess: async () => {  
              ...
              Actor.agentOf(EXTC_backend).replaceIdentity(
                  authClient.getIdentity()
              );  

              //registra a identidade do usuário no Canister de ledger
              Actor.agentOf(EXTC_icrc1_ledger_canister).replaceIdentity(
                  authClient.getIdentity()
              );              
  ...
  async function getAccountFromBalance(account) {
    ...
    try {
        if (account != '') {                        
            setFrom(account);
            const result = await EXTC_backend.getBalance(Principal.fromText(account));
  ...
  async function transferTokens() {        
    ...
    try {
        ...
        */
        ...
        const result = await EXTC_backend.transferFrom(Principal.fromText(to), BigInt(amount)); // esta operação consome 10000 tokens (operação de aprovação e transferencia juntas custam 20000)
    ...
    <div className="transfer-container">
      ...
      <div className="input-group">
        ...
        <div className="inline-fields">
          ...
          <a onClick={login} className="btn-login">Internet Identity</a>
        </div>        
